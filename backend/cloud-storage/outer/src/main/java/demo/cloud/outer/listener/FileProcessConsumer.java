...
public class FileProcessConsumer {
    ...
    public void handleFile(FileUploadEvent event, Channel channel, Message message) {
        ...
        log.info("消费到文件处理任务: ossKey={}, userFileId={}", event.getOssKey(), event.getPhysicalId());
        ...
        try {
            ...
            if (!isMimeAllowed(mimeType)) {
                log.info("MIME 类型不符合白名单，跳过 RAG。userFileId: {}", event.getPhysicalId());
                // 更新数据库状态为 "SKIPPED"
                // 手动确认消息（从队列移除）
            ...
            log.info("RAG 处理成功, userFileId: {}", event.getPhysicalId());

            // 4. 【手动 ACK】确认消息已处理
            channel.basicAck(deliveryTag, false);

        } catch (Exception e) {
            log.error("处理文件失败, userFileId: {}, error: ", event.getPhysicalId(), e);
            try {
                /*
    ...
    private boolean isMimeAllowed(String mimeType) {
        ...
        return allowedMimeTypes.stream().anyMatch(m -> mimeType.equalsIgnoreCase(m));
    }
}